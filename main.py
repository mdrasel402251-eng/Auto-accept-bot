...
from aiogram.filters import Command, ChatMemberUpdatedFilter, LEAVE_TRANSITION
...
conn = sqlite3.connect("auto_robot_v5.db", check_same_thread=False)
cursor = conn.cursor()
...
def db_init():
    ...
    cursor.execute("""
        INSERT OR IGNORE INTO stats VALUES ('leaves', 0)
    """)
...
def main_menu():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=USER_BUTTON_TEXT)]
        ],
...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "I can help you manage your Telegram channels by automatically approving new members. 🚀\n\n"
        "🛠 How to use me?\n"
...
async def admin_panel(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer(
            f"❌ Access Denied! You are not the authorized Admin.\n"
            f"Your Current Telegram ID: {message.from_user.id}\n\n"
            f"If this is your correct ID, please update the ADMIN_ID in the code."
        )
...
async def show_analysis(call: types.CallbackQuery):
    ...
    cursor.execute(
        "SELECT count FROM stats WHERE key='accepted'"
    )
    a_count = cursor.fetchone()[0]

    cursor.execute(
        "SELECT count FROM stats WHERE key='leaves'"
    )
...
async def show_active_channels(call: types.CallbackQuery):
    ...
    cursor.execute("SELECT chat_id, link FROM links")
    rows = cursor.fetchall()

    if not rows:

        await call.message.edit_text(
            "📺 No active channels found in the database yet. ❌",
            reply_markup=admin_kb()
        )
    ...
    for chat_id, link in rows:
        ...
        try:
            ...
            channel_name = f"Unknown Channel ({chat_id})"
    ...
    if len(channels_text) > 4096:
        channels_text = channels_text[:4000] + "\n...and more!"
...
async def ask_broadcast(call: types.CallbackQuery):
    ...
    await call.message.answer(
        "📢 Broadcast System \n\n"
        "To send a message to all users, use the format: \n"
...
async def auto_approve(request: ChatJoinRequest):
    ...
    try:
        ...
        cursor.execute(
            "SELECT link FROM links WHERE chat_id=?",
            (request.chat.id,)
        )
        ...
        accepted_msg = (
            f"🎉 Congratulations! {request.from_user.first_name}\n\n"
            f"Your request to join {request.chat.title} has been Automatically Approved! ✅\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "🚀 What is Next?\n"
            "Feel free to explore our content and stay active in the community. "
            "We are excited to have you with us! 🤝\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "⚠️ Important: Make sure you have started the bot by typing /start "
...
async def on_leave(event: ChatMemberUpdated):
    ...
    try:
        ...
        link = row[0] if row else await event.chat.export_invite_link()

        leave_msg = (
            f"👋 We are Sorry to See You Go!\n\n"
            f"You have recently left {event.chat.title}. "
            f"If this was a mistake or you changed your mind, "
            f"you are always welcome back! 🤝\n\n"

            f"🔗 Permanent Join Link:\n{link}\n\n"

            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "💡 Pro Tip: Keep this bot started by typing /start "
            "to get re-join links anytime you need. ⚡\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━"
        )
...
@dp.message(lambda message: message.text == USER_BUTTON_TEXT)
async def total_stats_handler(message: types.Message):
    ...
    await message.answer(
        f"📊 Bot Statistics \n\n"
        f"Total members accepted so far: {count} ✅ \n\n"
        f"Keep using /start to stay updated! 🔔"
    )
...
async def main():
    ...
    print("✅ Auto Approval Bot is online successfully!")
